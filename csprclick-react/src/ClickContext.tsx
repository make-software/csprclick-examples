...
export const ClickProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const _checkActiveAccount = async (ref: ICSPRClickSDK) => {
            try {
                const account = await ref.getActiveAccountAsync({ withBalance: true, withFiatCurrency: 'USD' });
    ...
    return (
        <ClickContext.Provider value={{ publicKey: connectedAccount?.public_key, provider: connectedAccount?.provider, clickRef }}>
            {children}
